...
public class ScatterInterpolator {
	...
	private int NUM_LINES = 13331;
	private float[] data;
	private float[] dates;
	private int lastIndex;
	public float startDate;
	public float finishDate;
	...
	*/
	public float getData(float date) {
		...
		if (date<dates[0] | date>dates[NUM_LINES-1]) {
			System.out.println("scatter interpolator input out of range");
			return 0.0f;
		}
		int index = 0;
		...
		float div = dates[index+1] - dates[index];
		float dif = date - dates[index];
	...
	*/
	private int hunt(float[] xx, int n, float x, int jlow) {
