...
*
*   Modified to include Linear Detrending Jan, 2003 lukas saul
*/
public class Spectrum {
	...
	*
	*   len is total length to read
	*
	*  include a skip for if the DIS has extra data to skip (every skip*4th byte we take)
	...
	*/
	public static double[] getAveragedFFTMagnitudes(
		...
		try {
			...
			// here's what we're working to return to the master
			...
			// load fist bit into temp array
			for (int i=0; i<halfSeg; i++) {
				temp2[i] = (double)dis.readInt();
			...
			for (int i=0; i<num; i++) {
				...
				// time to adjust the data with a window
				for (int k=0; k<numPerSegment; k++) {
					data[k]*=winCoeff[k];
				}
				...
				Complex[] fComps = FourierMath.transform(dat);
	...
	*/
	public static final void main(String[] args) {
		...
		file f = new file("z_efluxVdoy_INT.dat");
		file fOut = new file("z_efluxVdoy_spec.dat");
		fOut.initWrite(false);
		double[] pdf=new double[0];
		try {
			pdf = getAveragedFFTMagnitudes(f, 512);
		}
		catch(Exception e) {e.printStackTrace();}
		for (int i=0; i<pdf.length; i++) System.out.println(""+pdf[i]);

		double[] freqs = getPositiveFrequencies(15.0/24.0/60.0,512);
