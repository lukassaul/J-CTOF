...
/**
* This takes the ASCII output of PHADataFixer1 and creates a binary data file of fluxes
*
*
*  Here we generate an array of fluxes at small time intervals. (DELTAT)
*
*  OK, lets just save ALL THE FLUXES, to keep it easy.
*  The file won't be that big, just 62*12000*4 bytes.  Old file formats saved at bottom of this file
*
* CURRENT FORMAT:
...
public class PHADataFixer3 {
	...
	*/
	...
	public static int DELTAT = 3600;
	public static double e = 1;
	public static String OUTPUT_FILE_NAME = "phadataWstats_1hr.bin";
	...
	public PHADataFixer3(String[] args) {
		try {
			numIntervals = 0;
			numTotalBins = 0;
			...
			while (!eof) {
				if ((line = inf.readLine()) == null) eof = true;
					...
					// just to have some output here...
					if (day2 != day) {
						day = day2;
						System.out.println("date: " + date+ " DAY: "+ day);
					}

					// here's the energy bin for this event
					int eq = Integer.parseInt(st.nextToken());

					// keep track of max & min energy bin found
					if (eq>maxE) maxE=eq;
					if (eq<minE) minE=eq;
					...
					if (date-DELTAT < lastDate) {
						...
						// ok, now really output fluxes to form database:

						// first let's find appropriate energy range:
						// by checking v/vsw

						/* SKIP SW STUFF NOW...  SOMETHING IS GOING WRONG
						float vsw = (float)sr.getV(lastDate, lastDate+DELTAT);
						//System.out.println("got vsw: " + vsw);
						short startBin = 100;
						short numBins = 0;
						if (vsw>0) {
							for (int i=0; i<62; i++) {
								float vvmin = (float)(vMin[i]/vsw);
								float vvmax = (float)(vMax[i]/vsw);
								//System.out.println("min/max: "+vvmin+" "+vvmax);
								if ( (vvmin >= 1.5) &&	(vvmax <= 4.0) ) {
									//if (fluxes[i]>0) {
										if (fluxes[i]==0) numZeroBins++;
										numTotalBins++;
										numBins++;
										if (i<startBin) startBin = (short)i;
									//}
								}
							}
						}
						//System.out.println("numbins: " + numBins + " vsw: " + vsw);

						// ok, we know what we want now, lets write the file
						if (numBins >0) {
							numIntervalsWithBins++;
							dos.writeInt(lastDate);
							dos.writeShort(startBin);
							dos.writeShort(numBins);
							for (int i=0; i<numBins; i++) {
								//System.out.println("startbin " +
								//	startBin  +" writing flux: " + 	fluxes[startBin+i]);
								dos.writeFloat((float)fluxes[startBin+i]);
							}
							intervals++;
							bins+=numBins;
						}
